...
from genocrowd.libgenocrowd.Galaxy import Galaxy
...
class LocalAuth(Params):
    ...
    def persist_user(self, inputs, ldap=False):
        ...
        if self.get_number_of_users() == 0:
            ...
            admin = False
            blocked = self.settings.getboolean('genocrowd', 'default_locked_account')
        ...
        if not ldap:
            ...
            # Concat genocrowd_salt + user_password + salt
            salted_pw = self.settings.get('genocrowd', 'password_salt') + inputs['password'] + salt
            # hash
        ...
        user_id = database.execute_sql_query(
            query, (ldap, inputs['fname'], inputs['lname'], inputs['username'],
                    inputs['email'], sha512_pw, salt, api_key, admin, blocked, Utils.humansize_to_bytes(self.settings.get("genocrowd", "quota"))), True)
        ...
        return {
            ...
            'admin': admin,
            'blocked': blocked,
            'quota': Utils.humansize_to_bytes(self.settings.get("genocrowd", "quota")),
            'apikey': api_key,
            'galaxy': None
        }
    ...
    def create_user_directories(self, user_id, username):
        ...
        userdir_path = "{}/{}_{}".format(
            self.settings.get("genocrowd", "data_directory"),
            user_id,
            username
        )
    ...
    def authenticate_user(self, inputs):
        ...
        if len(rows) > 0:
            concat = self.settings.get('genocrowd', 'password_salt') + inputs['password'] + rows[0][7]
            if len(rows[0][6]) == 64:
                # Use sha256 (old genocrowd database use sha256)
        ...
        if not inputs["newPassword"] == '':
            if password_identical:
                ...
                if not authentication['error']:
                    ...
                    salted_pw = self.settings.get('genocrowd', 'password_salt') + inputs['newPassword'] + salt
