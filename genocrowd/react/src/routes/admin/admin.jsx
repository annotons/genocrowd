    ...
    axios.post(requestUrl, data, { baseURL: this.props.config.proxyPath, cancelToken: new axios.CancelToken((c) => { this.cancelRequest = c }) })
      .then(response => {
        ...
        this.setState({
          ...
          users: update(this.state.users, { [index]: { isAdmin: { $set: newAdmin } } })
        })
      })
      .catch(error => {
    ...
    let columns = [{
      ...
      formatter: (cell, row) => {
        return (
          <FormGroup>
            <div>
              <CustomInput type="switch" username={row.username} id={'set-admin-' + row.username} name="isAdmin" onChange={this.handleChangeAdmin} label="Admin" checked={cell} value={cell} />
            </div>
          </FormGroup>
        )
      },
